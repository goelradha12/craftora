/* ============================================================
   customize.js — Craftora Product Customizer (canvas engine)
   Ported from src/pages/fragments/customize.inline.js (originally
   client/customize.html inline <script>). Drag/resize/select/
   template/text/upload logic is UNCHANGED from the original —
   only the outer wiring was adapted:
     - window load / DOMContentLoaded listeners -> direct calls,
       since this now runs from a React useEffect after the
       fragment markup is already in the DOM.
     - onclick="..." attributes in the fragment HTML look up
       functions on window, so every function referenced that way
       is explicitly assigned to window inside initCustomize().
   Returns a cleanup function that removes the window.* globals it
   registered, for use in a React useEffect.
   ============================================================ */
...
export function initCustomize() {
        ...
        async function generateSnapshot(side) {
            const targetSide = side || state.side;
            ...
            for (const layer of (state[targetSide] || [])) {
        ...
        async function saveDesign() {
            ...
            const cfg = getCfg();
            const hasFrontBack = cfg.sides.includes('front') && cfg.sides.includes('back');
            let previewImage = null, previewImageFront = null, previewImageBack = null;
            try {
                if (hasFrontBack) {
                    previewImageFront = await generateSnapshot('front');
                    previewImageBack = await generateSnapshot('back');
                    previewImage = previewImageFront;
                } else {
                    previewImage = await generateSnapshot(state.side);
                }
            } catch (e) { console.warn('Snapshot failed:', e); }
            const data = { productId: pid || null, productCategory: state.category, productName: state.product && state.product.name || null, shirtColor: state.shirtColor, activeSide: state.side, front: serializeLayers(state.front), back: serializeLayers(state.back), wrap: serializeLayers(state.wrap), generatedAt: new Date().toISOString(), previewImage: previewImage, previewImageFront: previewImageFront, previewImageBack: previewImageBack };
            try {
                ...
                try { data.previewImage = null; data.previewImageFront = null; data.previewImageBack = null; localStorage.setItem(key, JSON.stringify(data)); markSaved(); showToast('Saved (preview skipped - storage full)'); }
        ...
        (function () {
            ...
            // Expose so reset/clear functions (and the AI Design panel) can call
            // these from outside this IIFE
            window._clearUploadUI = clearUploadUI;
            window._placeOnCanvas = placeOnCanvas;
        })();

        /* ══════════════════════════════════════════════
           AI DESIGN ASSISTANT PANEL
           UI/event-glue only — Groq (prompt) and HuggingFace
           (image) networking lives in services/aiPromptService.js
           and services/aiImageService.js.
        ══════════════════════════════════════════════ */
        (function () {
            const promptEl = $('#ai-prompt-input');
            const styleSelectEl = $('#ai-style-select');
            ...
            generateBtn.addEventListener('click', async function () {
                const ideaText = (promptEl?.value || '').trim();
                if (!ideaText) {
                    showError('Description required', 'Please describe the design you want to generate.');
                    return;
                }

                const productLabel = getCfg().label;
                ...
                try {
                    ...
                    showError('Design generation failed', err.message || 'Please try again in a moment.');
                }
            });
        })();
    ...
    window.setShirtColor = setShirtColor;
    window.setTextAlign = setTextAlign;
    window.addTextLayer = addTextLayer;
    window.removeSel = removeSel;
    window.centerSel = centerSel;
    window.alignSel = alignSel;
    window.clearSide = clearSide;
    window.promptResetDesign = promptResetDesign;
    window.confirmResetDesign = confirmResetDesign;
    window.handleBack = handleBack;
    window.openModal = openModal;
    window.closeModal = closeModal;
    window.saveAndBack = saveAndBack;
    window.proceedBack = proceedBack;
    window.saveDesign = saveDesign;
    window.handleToolbarCart = handleToolbarCart;
    window.handleClearDesign = handleClearDesign;

    return function cleanupCustomize() {
        delete window.switchSide;
        delete window.togglePanel;
        delete window.setShirtColor;
        delete window.setTextAlign;
        delete window.addTextLayer;
        delete window.removeSel;
        delete window.centerSel;
        delete window.alignSel;
        delete window.clearSide;
        delete window.promptResetDesign;
        delete window.confirmResetDesign;
        delete window.handleBack;
        delete window.openModal;
        delete window.closeModal;
        delete window.saveAndBack;
        delete window.proceedBack;
        delete window.saveDesign;
        delete window.handleToolbarCart;
        delete window.handleClearDesign;
        delete window._clearUploadUI;
        delete window._placeOnCanvas;
    };
}
